...
final Networkresponce responce =await Networkcoller().getrequest(Urls.updatedtaskstatus(widget.task.id??"", status));
...
widget.showprogress(false);
  ...
  Future<void>delettask(String status)async {
    widget.showprogress(true);
    final Networkresponce responce =await Networkcoller().getrequest(Urls.updatedtaskstatus(widget.task.id??"", status));
    if(responce.isSuccess){
      widget.onstatuschange();
  ...
  Widget build(BuildContext context) {
    return Card(
      child: Column(
        ...
        children: [
          Text(widget.task.title??"",style: TextStyle(fontSize: 20,fontWeight: FontWeight.w600),),
          Text(widget.task.description??""),
          Text("Date: ${widget.task.createdDate}"),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Chip(label: Text(widget.task.status??"new",style: TextStyle(fontWeight: FontWeight.bold,),),backgroundColor: Colors.green,),
              Wrap(
                children: [
